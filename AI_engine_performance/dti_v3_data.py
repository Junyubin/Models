...
config = {'limit' : 10000, 'normal_days' : 365, 'attack_days' : 365, 'predict_time' : 365, 'index_cols' : 'lgtime, src_ip, dst_ip'}
# config = Params.get_config('COMMON', '335', 'dti_v3_model')
...
class DataCreation():
    ...
    def train_data_load(self):
        ...
        print('******* 공격 데이터 불러오기 *******')
#         sql_array = attack_query(self.attack_start_time, self.end_time, self.data_limit)
        
        attack_data = pd.DataFrame()
        for attack in self.attack_array:
            sql = attack_query(attack, self.attack_start_time, self.end_time, self.data_limit * 4, self.interval)
            try:
                result, meta = execute_ch(sql, with_column_types = True)
            except:
                print('ERROR: CHECK THE ATTACK DATA QUERY...')
                print(attack_query(attack, self.attack_start_time, self.end_time, self.data_limit * 4, self.interval))
                return
            if not result:
