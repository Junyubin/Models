...
def train_test_split(indexes, data, labels, tr_split=0.7, vl_split=0.9):
    permutation = np.random.permutation(len(indexes))
    train_size = int(len(indexes) * tr_split)
    valid_size = int(len(indexes) * vl_split)
    data_dict = {}
    data_dict['IDX_train'] = indexes.values[permutation[:train_size]]
    data_dict['IDX_valid'] = indexes.values[permutation[train_size: valid_size]]
    data_dict['IDX_test'] = indexes.values[permutation[valid_size:]]
    data_dict['X_train'] = data.values[permutation[:train_size]]
    data_dict['Y_train'] = labels[permutation[:train_size]]
    data_dict['X_valid'] = data.values[permutation[train_size: valid_size]]
    data_dict['Y_valid'] = labels[permutation[train_size: valid_size]]
    data_dict['X_test'] = data.values[permutation[valid_size:]]
    data_dict['Y_test'] = labels[permutation[valid_size:]]
    data_dict['X_train'] = data_dict['X_train'].reshape(data_dict['X_train'].shape[0], data_dict['X_train'].shape[1], 1)
    data_dict['X_valid'] = data_dict['X_valid'].reshape(data_dict['X_valid'].shape[0], data_dict['X_valid'].shape[1], 1)
...
class Params():
    ...
    def set_logger(mode, model_id, defult=False):
        ...
        logger = logging.getLogger('dti_v3_logger')
